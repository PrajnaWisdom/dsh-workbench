...
// ---- built-in "黄金实时走势" panel (id __gold__) ----
// Same one-shot seeding as the token panel: deleting it is final, a restart
// does NOT recreate it.
const GOLD_PANEL_ID = '__gold__'

function seedGoldPanel() {
  try {
    const dir = dashboardsDir()
    const marker = join(dir, '.seeded-gold')
    if (existsSync(marker)) return
    mkdirSync(dir, { recursive: true })
    const target = join(dir, GOLD_PANEL_ID, 'dashboard.json')
    if (!existsSync(target)) {
      writeDashboard({
        id: GOLD_PANEL_ID,
        type: 'gold',
        title: '黄金实时走势',
        description: '国际现货黄金 XAU/USD 实时价格',
        order: 0,
        createdAt: 0,
      })
    }
...
// ---- live gold spot price (keyless api.gold-api.com) ----
...
function readAll() {
  ...
  out.sort((a, b) => {
    ...
    const ar = builtinRank(a)
    const br = builtinRank(b)
...
// Order key: the built-in panels are seeded with 0, user dashboards get
...
// When two dashboards share the same order (both built-ins seeded at 0), keep
// token first, gold second, user dashboards last. Once the user reorders, every
// order becomes unique and this tiebreaker no longer applies.
function builtinRank(d) {
  if (d.type === 'token') return 0
  if (d.type === 'gold') return 1
  return 2
}
...
function apply(ctx) {
  // Seed the built-in panels exactly once (first install); once deleted, a
  // restart does not recreate them.
  seedTokenPanel()
  seedGoldPanel()
  ...
  ctx.effect(() => {
    const disposers = [
      ...
      ctx.webServer.register({
        ...
        path: API.gold,
        handler: async (_req, res) => {
          writeJson(res, 200, await fetchGoldPrice())
        },
      }),
      ctx.webServer.register({
        kind: 'exact',

...
// ---- cleanup: remove a __gold__ panel seeded by an earlier "built-in panel"
// implementation (it is now a regular widget instead, not a seeded dashboard). ----
function cleanLegacyGoldPanel() {
  try {
    const target = join(dashboardsDir(), '__gold__')
    if (!existsSync(target)) return
    const raw = readFileSync(join(target, 'dashboard.json'), 'utf8')
    const d = JSON.parse(raw)
    if (d && d.type === 'gold') rmSync(target, { recursive: true, force: true })
  } catch {
    /* ignore */
  }
}
...
function apply(ctx) {
  ...
  const widgetSchema = {
    ...
    properties: {
      type: { type: 'string', required: true, enum: ['markdown', 'metrics', 'table', 'text', 'gold'], description: '组件类型：markdown 富文本 / metrics 指标卡组 / table 数据表 / text 纯文本 / gold 实时金价（国际现货黄金 XAU/USD，含走势曲线，每 30 秒自动刷新）' },
